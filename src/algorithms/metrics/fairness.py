...
def score_rule(
    rule: Prescription,
    solution: List[Prescription],
    covered: Set[int],
    covered_protected: Set[int],
    idx_protec,
    unprotected_coverage_threshold: float,
    protec_cvrg_th: float,
) -> float:
    """
    ...
    protec_cvrg_factor = (
        (len(new_covered_protec) / len(idx_protec)) / protec_cvrg_th
        if protec_cvrg_th > 0
        else 1
    )
    unprotec_cvrg_factor = (
        (len(new_covered - new_covered_protec) / (len(rule.covered_idx - idx_protec)))
        / unprotected_coverage_threshold
        if unprotected_coverage_threshold > 0
        else 1
    )

    # Use the minimum of the two coverage factors
    coverage_factor = min(protec_cvrg_factor, unprotec_cvrg_factor)

    score = rule.utility * coverage_factor

    logging.debug(
        f"Rule score: {score:.4f} (expected utility: {exp_util:.4f}, expected protected utility: {protec_exp_util:.4f}, "
